...
$(APP_BUNDLE_OUT): $(APP_BUNDLE_SRC) src/app-media-utils.mjs src/app-bible-reference-utils.mjs src/app-pptx-utils.mjs src/app-controls-utils.mjs src/app-media-loading-utils.mjs src/app-ui-templates.mjs | $(DERIVED_DIR)
ifeq ($(WINDOWS), 1)
	@powershell -NoProfile -c "New-Item -ItemType Directory -Force -Path '$(dir $@)'" >nul 2>&1
else
	@mkdir -p $(dir $@)
