...
function showPreviewWarningToast() {
    ...
    toast.textContent = "Press \"Start Presentation\" to show on the selected display.";
...
async function handleMediaWindowClosed(event, id) {
    ...
    if (currentMode === MEDIAPLAYER && mediaPlayerInputState.filePaths.length > 0) {
...
function waitForMetadata() {
    if (!video || !video.src || video.src === '' || isLiveStream(video.src) || isImg(video.src)) {
